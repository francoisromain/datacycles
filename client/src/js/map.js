...
var formatTimeString = function(input, formatInput, formatOutput){
...
var showDockInfo = function (d, dock) {
    ...
    .each(function(d) {
      for (var i = 0; i < d.properties.activity.length; i++) {
        ...
        if (changeTime < realtime) {
          d.properties.currentQty = d.properties.activity[i].bikes_available;
        }
      }
    })
    .attr({
      transform: function (d) { 
        return "translate(" + -3 + "," + -(d.properties.currentQty + 7) + ")";
      }, 
      height: function (d) { return d.properties.currentQty; }
    });
};
